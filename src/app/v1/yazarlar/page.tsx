...
export default async function V1AuthorsPage() {
  const authors = await getAuthors();
  return (
    <PublicShell v1>
      <main className="page container">
        <section className="page-title">
          <span className="eyebrow">Müellifler</span>
          <h1>Yazarlarımız</h1>
        ...
        <section className="authors-grid">
          {authors.map((author) => (
            <article className="author-card" key={author.id}>
              <Link href={`/v1/yazar/${author.id}`}><img src={author.photo ?? "/logo.jpg"} alt={author.name} /></Link>
              <h2><Link href={`/v1/yazar/${author.id}`}>{author.name}</Link></h2>
